...
_ = raw_input()  # Not used
types = map(int, raw_input().split())
type_count = [0] * 6  # 0 (not used), 1-5 bird types

bird_type = max_birds = 0

for t in types:
    type_count[t] += 1

    if type_count[t] > max_birds:
        max_birds = type_count[t]
        bird_type = t
    elif type_count[t] == max_birds:
        if t < bird_type:
            bird_type = t
